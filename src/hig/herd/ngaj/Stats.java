...
public class Stats extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Declare to double arrays where the total distance and steps for
		// previous seven days are saved.
		double values1[] = new double[7];
		double values2[] = new double[7];

		/**
		 * Fill values1 and values2 arrays with total distance and total steps
		 * from previous seven days. also fill ValuesX array with date and month
		 * of previous seven days.
		 * 
		 * For each cycle call method decreasedate which updates the values of
		 * date,month and year variables by decreasing the date by 1.
		 */
		for (int i = 6; i >= 0; i--) {
			double TotalDistance = 0;
			try {
				Cursor cr1 = db.rawQuery(
						"Select SUM(Distance) as TotalDistance From tblTracks Where Date='"
								+ year + "-" + month + "-" + date + "'", null);
				if (cr1.moveToFirst()) {
					TotalDistance = (double) cr1.getFloat(cr1
							.getColumnIndex("TotalDistance"));
				}
				values1[i] = Double.parseDouble(new DecimalFormat("#")
			...
			try {
				double TotalSteps = 0;
				Cursor cr1 = db.rawQuery(
						"Select SUM(Steps) From tblTracks Where Date='" + year
								+ "-" + month + "-" + date + "'", null);
				if (cr1.moveToFirst()) {
					TotalSteps = cr1.getInt(0);
				}
				values2[i] = Double.parseDouble(new DecimalFormat("#")
						.format(TotalSteps));
			} catch (Exception ex) {
		...
		/**
		 * If device has small screen size then only the Total Distance graph is
		 * drawn and screen orientation is locked to landscape. Else both graphs
		 * are drawn using DrawGraphs Method.
		 */
	...
	/**
	 * This method updates the values of day,month and year variables by
	 * decreasing the day by one It takes care when passing from one month/year
	 * to other and also deals with months with different length.
	 */
	...
	/**
	 * This method draws graphs on the layout given as parameter using GraphView
	 * library. Also Y values and graph title are passed as parameters while X
	 * values are read from ValuesX array.
	 */
	public void DrawGraphs(LinearLayout layout, double[] values, String title) {
		...
		Arrays.sort(values);
		double min = Double.parseDouble(new DecimalFormat("#")
				.format(values[0]));
		double max = Double.parseDouble(new DecimalFormat("#")
				.format(values[6]));
		double middle = Double.parseDouble(new DecimalFormat("#")
				.format((max + min) / 2));
		double m1 = Double.parseDouble(new DecimalFormat("#")
				.format((middle + max) / 2));
		double m2 = Double.parseDouble(new DecimalFormat("#")
				.format((middle + min) / 2));
		String a[] = { String.valueOf(max), String.valueOf(m1),
				String.valueOf(middle), String.valueOf(m2), String.valueOf(min) };
