...
public class DBservice extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		/**
		 * Save the just read values into local database if needed create the
		 * database and tables.
		 */
		...
		db.execSQL("Insert into tblTracks VALUES ((SELECT max(ID) FROM tblTracks)+1,'"
				+ Name
				+ "',"
				+ Distance
				+ ",'"
		...
		for (int j = 0; j < size; j++) {
			...
			db.execSQL("Insert into tblPoints Values ((SELECT max(PointID) FROM tblPoints)+1,"
					+ id + "," + lat + "," + lng + ");");
		}
	}
